...
alpha_ij = non_randomness_parameters.ijs(
    property=f"alpha | {components[0]} | {components[1]}",
    res_format='alphabetic'
)
print(type(alpha_ij))
print(alpha_ij)
...
dg_ij, dg_ij_comp, dg_ij_comp_upd = calc_dg_ij_using_nrtl_model(
...
print(dg_ij)
print(dg_ij_comp)
print(dg_ij_comp_upd)

# # NOTE: calculate the interaction parameter matrix (tau_ij)
tau_ij, tau_ij_comp, tau_ij_comp_upd = calc_tau_ij_using_nrtl_model(
    components=[methanol, ethanol],
    temperature=temperature,
    dg_ij=dg_ij_comp,
)
